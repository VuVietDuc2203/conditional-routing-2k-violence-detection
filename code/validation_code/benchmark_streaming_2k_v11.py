...
TELEMETRY_FIELDS = [
    "elapsed_sec", "source_time_sec", "measurement", "gpu_util_percent",
    "memory_used_mb", "power_w", "temperature_c",
]
...
def integrate_power_joules(rows: list[dict[str, Any]]) -> tuple[float | str, float | str]:
    """Trapezoidal GPU-board energy and coverage over measurement-only samples."""
    points = [
        (float(row["elapsed_sec"]), float(row["power_w"]))
        for row in rows
        if row.get("measurement") is True
        and isinstance(row.get("elapsed_sec"), (int, float))
        and isinstance(row.get("power_w"), (int, float))
    ]
    if len(points) < 2:
    ...
    return energy_j, points[-1][0] - points[0][0]
...
def checkpoint_for(args: argparse.Namespace) -> Path | None:
    if args.mode == "m3_gate_only":
        return None
    m3_modes = {"m3_gated", "m3_route_on", "m3_route_bypassed"}
    root = args.m3_root if args.mode in m3_modes else args.m1_root
    variant = "M3" if args.mode in m3_modes else "M1"
...
def run_once(args: argparse.Namespace, repeat: int) -> dict[str, Any]:
    ...
    cfg = RuntimeConfig(threshold=args.threshold, hash_candidates=args.hash_candidates)
    ...
    try:
        while True:
            ...
            record: dict[str, Any] = {
                ...
                "eligible_candidates": 0, "route_selected_candidates": 0,
                "candidate_ids": "", "candidate_sha256s": "",
            ...
            if source_time + 1e-9 >= next_analysis_at:
                ...
                record.update({key: float(timing.get(key, 0.0)) for key in
                               ("yolo_ms", "yolo_inference_ms", "hdbscan_ms", "gate_ms", "crop_ms")})
                candidates = result.get("candidates", [])
                record["eligible_candidates"] = int(result.get("eligible_candidates", len(candidates)))
                record["route_selected_candidates"] = int(result.get("route_selected_candidates", 0))
                record["candidate_ids"] = "|".join(str(candidate["candidate_id"]) for candidate in candidates)
                record["candidate_sha256s"] = "|".join(str(candidate["tensor_sha256"]) for candidate in candidates)
            ...
            if telemetry_elapsed >= next_telemetry_at:
                while next_telemetry_at <= telemetry_elapsed:
                    next_telemetry_at += 1.0
                telemetry.append({
                    "elapsed_sec": telemetry_elapsed,
                    "source_time_sec": source_time,
                    "measurement": source_time >= args.warmup_sec,
                    **gpu_telemetry(),
                })
            trace.append(record)
            frame_index += 1
    ...
    measured_telemetry = [row for row in telemetry if row.get("measurement") is True]
    gpu_board_energy_j, energy_coverage_sec = integrate_power_joules(telemetry)
    summary = {
        **manifest,
        ...
        "eligible_candidates": sum(row["eligible_candidates"] for row in measured),
        "route_selected_candidates": sum(row["route_selected_candidates"] for row in measured),
        "q_candidate": sum(row["classifier_calls"] for row in measured) / max(1, sum(row["eligible_candidates"] for row in measured)),
        "q_update": sum(row["classifier_calls"] for row in measured) / max(1, len(measured)),
        ...
        "gpu_util_mean_percent": statistics.mean(numeric_telemetry(measured_telemetry, "gpu_util_percent")) if numeric_telemetry(measured_telemetry, "gpu_util_percent") else "not_available",
        "vram_peak_mb": max(numeric_telemetry(measured_telemetry, "memory_used_mb"), default="not_available"),
        "power_mean_w": statistics.mean(numeric_telemetry(measured_telemetry, "power_w")) if numeric_telemetry(measured_telemetry, "power_w") else "not_available",
        "power_peak_w": max(numeric_telemetry(measured_telemetry, "power_w"), default="not_available"),
        "gpu_board_energy_j": gpu_board_energy_j,
        "gpu_board_energy_j_per_update": (
            gpu_board_energy_j / len(measured)
            if isinstance(gpu_board_energy_j, float) and measured
            else "not_available"
        ),
        "gpu_board_energy_coverage_sec": energy_coverage_sec,
        "gpu_board_energy_scope": "GPU board only; trapezoidal integration of measurement-only nvidia-smi samples",
    }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--mode",
        choices=["m1_dense_s1", "m1_stride50", "m3_gated", "m3_gate_only", "m3_route_on", "m3_route_bypassed"],
        required=True,
    )
    ...
    parser.add_argument("--hash-candidates", action=argparse.BooleanOptionalAction, default=True)
